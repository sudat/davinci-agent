...
from services.gates import PHASE_0C_CRITERIA, PHASE_0C_FIXTURES, GatePolicy, GateResult
from services.gates.serialization import canonical_gate_bytes
from services.toolchain.models import LockError, Phase0CToolchainLock, load_lock
from services.toolchain.verify import verify_binary

PHASE_0C_GOLDEN_DIR = Path("tests/goldens/reference/phase-0c")
PHASE_0C_MANIFEST_DIR = Path("tests/fixtures/manifests/phase-0c")


@dataclass(frozen=True, slots=True)
class Phase0CPrepareRequest:
    toolchain_lock: Path
...
def _load_manifest(root: Path, fixture_id: str) -> tuple[Phase0CFixtureManifest, bytes]:
    path = root / PHASE_0C_MANIFEST_DIR / f"{fixture_id}.json"
    raw = path.read_bytes()
...
def _golden_hashes(root: Path, manifests: dict[str, bytes]) -> GoldenHashes:
    phase_dir = root / PHASE_0C_GOLDEN_DIR
...
def _assert_golden_agreement(
    ...
    for fixture_id, raw in manifests.items():
        ...
        manifest = Phase0CFixtureManifest.model_validate_json(raw)
        outcome = manifest.expected
        if (
            golden.get("classification") != outcome.classification
            or golden.get("decision") != outcome.decision
            or golden.get("resulting_plan_version") != outcome.resulting_plan_version
            or golden.get("target_candidate_item_ids")
            != list(outcome.target_candidate_item_ids)
        ):
            raise PrepareError(f"manifest/golden outcome drift: {fixture_id}")
...
def _verify_parent_gate_result(path: Path, root: Path) -> tuple[str, str]:
    ...
    frozen_0b_policy = root / "config/gates/phase-0b-v1.json"
    policy_sha256 = sha256_file(frozen_0b_policy)
    if result.gate_id != "phase-0b" or result.gate_version != "v1":
        raise PrepareError("parent gate result must be the frozen phase-0b v1 result")
    if not result.passed:
        raise PrepareError("parent phase-0b gate result did not pass")
    if result.policy_sha256 != policy_sha256:
        raise PrepareError("parent gate result is bound to a different phase-0b policy")
...
def verify_phase0c_toolchain(lock_path: Path) -> Phase0CToolchainLock:
    try:
        ...
        if not isinstance(lock, Phase0CToolchainLock):
            raise PrepareError("phase-0c requires the phase-0c toolchain lock")
        if (
            ...
            or lock.smoke.preview_review.status != "passed"
        ):
            raise PrepareError("incomplete Phase 0C toolchain smoke results")
        if lock.preview_review.adapter != "pinned-ffmpeg-preview":
            raise PrepareError("phase-0c preview adapter must be the pinned ffmpeg")
        if lock.preview_review.external_model != "none":
            raise PrepareError("phase-0c must not pin an external model")
        verify_binary(lock.ffmpeg.ffmpeg)
        verify_binary(lock.ffmpeg.ffprobe)
    except LockError as error:
        raise PrepareError(f"phase-0c toolchain lock rejected: {error}") from error
    return lock


def prepare_phase0c(request: Phase0CPrepareRequest) -> FreezeIntent:
    if request.fixture_ids != PHASE_0C_FIXTURES:
        raise PrepareError("phase-0c accepts exactly the five canonical fixture ids")
    ...
    receipt = Phase0CFreezeReceipt(
        ...
        fixture_manifests=tuple(
            ManifestBinding(
                ...
                path=str(
                    (root / PHASE_0C_MANIFEST_DIR / f"{fixture_id}.json").resolve(strict=True)
                ),
...
def prepare_phase0c_errors() -> tuple[type[Exception], ...]:
