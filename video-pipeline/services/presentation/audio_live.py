"""LIVE end-to-end audio driver CLI (Todo 59): owned project, shared mix, QC.

Builds one owned ``__fvp_test__`` timeline whose program audio is the
SHARED external mixed derivative (pinned ffmpeg render of the profile's
approved BGM/SE over the edit-source bed, normalized toward the declared
loudness/peak targets), renders through the official Deliver API, and
verifies loudness/peak/channel compliance from the rendered output bytes.
The Fairlight preset rung is honored only when the published probe findings
verify apply+readback; otherwise the ladder resolves to the external mix
(default) — never to unverified automation. Every run deletes only the
owned project and always leaves Resolve running.
"""

from __future__ import annotations

import argparse
import sys
from pathlib import Path
from typing import Final, cast

from services.contracts.primitives import RationalFrameRate
from services.fixtures.manifest_phase3 import Phase3FixtureManifest
from services.presentation.audio_live_flow import MANIFEST_DIR_NAME, AudioLiveFlow
from services.presentation.audio_live_media import MARKER, AudioLiveError
from services.resolve_bridge.connection import BridgeConnectionError, connect
from services.resolve_bridge.lifecycle import cleanup_owned_projects
from services.resolve_bridge.readiness import load_host_report

EXIT_UNAVAILABLE: Final = 4
BRAND_MANIFEST: Final = Path(MANIFEST_DIR_NAME) / "p3-brand-a.json"


def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser()
    parser.add_argument("--report", type=Path, required=True)
    parser.add_argument("--evidence", type=Path, default=None)
    parser.add_argument("--ffmpeg", type=Path, default=None)
    parser.add_argument("--ffprobe", type=Path, default=None)
    parser.add_argument("--base-source", type=Path, default=None)
    parser.add_argument("--timeout", type=float, default=900.0)
    return parser


def main() -> int:
    arguments = _parser().parse_args()
    missing = [
        name
        for name, value in (
            ("--evidence", arguments.evidence),
            ("--ffmpeg", arguments.ffmpeg),
            ("--ffprobe", arguments.ffprobe),
            ("--base-source", arguments.base_source),
        )
        if value is None
    ]
    if missing:
        print(f"required: {missing}", file=sys.stderr)
        return 2
    evidence: Path = arguments.evidence
    bundle = evidence / "audio-live"
    bundle.mkdir(parents=True, exist_ok=True)
    try:
        report = load_host_report(arguments.report)
        connection = connect(report)
    except (BridgeConnectionError, OSError) as error:
        print(f"{MARKER} bridge unavailable: {error}", file=sys.stderr)
        return EXIT_UNAVAILABLE
    manifest = Phase3FixtureManifest.model_validate_json(BRAND_MANIFEST.read_bytes())
    flow = AudioLiveFlow(
        connection=connection,
        bundle=bundle,
        ffmpeg=cast("Path", arguments.ffmpeg),
        ffprobe=cast("Path", arguments.ffprobe),
        base_source=cast("Path", arguments.base_source),
        manifest=manifest,
        frame_rate=RationalFrameRate(num=30, den=1),
        render_deadline=arguments.timeout,
    )
    try:
        flow.run()
    except (AudioLiveError, OSError, ValueError) as error:
        print(f"{MARKER} FAIL {error}", file=sys.stderr)
    finally:
        try:
            cleanup_owned_projects(connection.project_manager())
        except Exception as error:  # noqa: BLE001 (cleanup must not mask failures)
            print(f"{MARKER} cleanup warning: {error}", file=sys.stderr)
    print(f"{MARKER} {'PASS' if flow.passed else 'FAIL'} rung={flow.rung}")
    return 0 if flow.passed else 1


if __name__ == "__main__":
    raise SystemExit(main())
