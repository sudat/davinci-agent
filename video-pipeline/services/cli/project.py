...
@dataclass(frozen=True, slots=True)
class ReviewSubtitle:
    """One review-plane subtitle cue input (id + segment + text + span)."""
...
def project_plan(  # noqa: PLR0913 (projection inputs are the review-plane contract)
    committed: EditPlan,
    ...
    *,
    ...
    subtitles: tuple[ReviewSubtitle, ...],
) -> ReviewPlane:
    ...
    plan = EditPlan0C(
        artifact_id=f"edit-plan-review-{episode_id}",
        ...
        plan=EditPlanBody0C(
            plan_version="v1",
            edit_source=EditSourceRef0C(source_id=source_id, total_frames=total_frames),
            items=tuple(items),
        ),
    )
