...
def ensure_av_proxy(*, ffmpeg: Path, mezzanine: Path, workspace_dir: Path) -> Path:
    """Build the low-res A/V proxy once per Edit Source (A/B recipe).

    ``ffmpeg -vf scale=480:-2 -c:v libx264 -crf 28 -c:a aac -b:a 96k
    -movflags +faststart`` — the exact recipe the A/B measured
    (25,662 VIDEO tokens ≈ $0.008/call at LOW resolution). Rebuilds only
    when the Edit Source size/mtime moved; the proxy is rebuildable
    runtime state under the episode dir.
    """
