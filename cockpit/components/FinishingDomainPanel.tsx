...
type FinishingDomainPanelProps = {
  ...
  /** 真のとき通常画面の1行警告だけを出す（manual/blockedが無ければnull）。
   *  偽（既定）のとき従来どおり7分野一覧を出す。 */
  summaryOnly?: boolean;
};

function isActionable(entry: FinishingDomainStatus): boolean {
  return entry.status === "manual_fallback_required" || entry.status === "blocked";
}
...
export default function FinishingDomainPanel({
  ...
  if (summaryOnly) {
    if (error !== null || payload === null || !payload.available) return null;
    const actionable = payload.domains.filter(isActionable);
    if (actionable.length === 0) return null;
    const labels = actionable.map((entry) => DOMAIN_LABEL[entry.domain] ?? entry.domain);
    const reasons = actionable.map(
      (entry) => statusExplanation(entry.status) ?? "記録された状態を確認してください。",
    );
    return (
      <div className="finishing-alert" data-testid="finishing-alert" role="alert">
        <p>
          確認が必要です：{labels.join("・")} — {reasons.join(" ")}
        </p>
      </div>
    );
  }
