...
該当IDなし。Inspectorプロパティ/MCP track state経路のみ）。キーボードで
届かない機能が実測で見つかったとき、初めて固定座標経路を検討する
（索引の共通原則8: API > .drt > キーボード > 座標クリック > AI CU。
座標ワークフローの先回り実装は本記録時点で見送り——Opus判断 2026-09-06）。
