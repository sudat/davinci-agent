...
### 2026-09-04: D提示(2階層テロップ+字幕再スタイル)の納品プロジェクト適用

suda承認のD提示を `v44-real-01-deliverable` へ部分適用した(プロジェクト削除・再作成
なし、V1/A1無触)。内容: 常時テロップのL1を白帯+濃色文字へ、新設のL2(章名
「どこにもないカメラバッグ」)をV4トラックに42タイルで配置(暗帯・白文字・右寄り)、
字幕を実機検証済みのHiragino Sans W5・0.055・影付きへ。冒頭見出しと章カードは無変更
(適用後レンダーで開幕2フレームが完全一致することを証明)。同一性証明: 適用後の
フルレンダーと承認済みr4見本を15フレームで比較し、文字縁の分散ノイズのみ
(worst mean 1.07/255・変化画素0.02%)。編集可能性: V4カードはV3と同一の入れ子カード
機構で、get_text_plusによる文字列読み取りを証明。手順書
(`private/runtime/sol-wbs3-deliverable-20260904/demo-steps.md`)にV4の1行を追加。
証拠: `private/runtime/sol-d-apply-20260904/`(適用記録は必須事項を冒頭に記載)、
見本系証拠 `private/runtime/sol-d-sample-{r2,r3,r4}-20260904/`。

**字幕カード枚数 100 → 98**。st68(「ま、あってもおかしくないんじゃないのかな」)が
4枚に語中分割されていたC-liveの不具合を、2枚(「ま、あっても」/「おかしくないんじゃ
ないのかな」)へ修正したため。**この変更はC(suda承認済み)の内容を変える**ものである
(文言は同一だが、カード枚数と区切り位置が変わる)。Opus要件により記録する。

**単語保全の限界**: 語の一体性を硬制約とする修正は `_WORD_UNIT_SPLITS`
(`services/creative_plan/subtitle_speed.py`、収録1件)の直値表で実現している。
**表に収録されていないテキストの語保全は保証されない**。判定には形態素解析を要し、
no-analyzer方針(Gate V44-2凍結)で導入を見送ったため。同種の不具合はレビューで
捕まえる。頻発するようになった時点で投資を再検討する。

**再生フレームレートの診断記録**: 適用前の音割れは
timelineFrameRate 30 / timelinePlaybackFrameRate 24 の不一致が原因(書き出しは正常、
再生のみ歪む)。`timelinePlaybackFrameRate` はAPIで書けない(api_truth: Resolve
Studio 21.0.2で全値形式False)。sudaが手動で30へ変更して解消済み。適用セッションは
この手動設定を3点(前・中・後)で検証し保持した(設定への書き込みなし)。**次回フル
ビルド(build_driverのproject削除→再作成経路)では再生24が戻る**。対策は「正しい
再生レートを持つプロジェクトの複製(第一候補・確実) vs プリセット(当該キーを運べるか
未観測)」をエピソード2着手時に比較して決める(現時点では未決・未実装)。
`perfRenderCacheMode="smart"` はAPIで書けるためprepare時に既定書き込みとした。

### 2026-09-06: キーボード割当の外部注入経路（実測）と共有Mac注意

キーマップ外部注入（Opus夜間タスク、suda承認・B案=空きキーのみ追加）の実測記録。
証拠一式: `private/runtime/sol-keymap-20260906/`（summary.md・format-analysis.md・
プリセット書き出し双方向・スクリーンショット76枚）。

**経路（唯一）**: キーボードカスタマイズダイアログ → `…`メニュー →
プリセットの書き出し/読み込み。**ブートストラップにGUI操作が1回だけ必要**
（それ以外は全てファイル層で完結する）。キーボードデータはディスク上のどこにも
ファイルとして存在せず、`User.db`（SQLite）の `SyConfig.KeyboardPresets` /
`SmGenericPresetList` / `SmPreset` テーブルも読み取り時点で空だった
（実行中のインメモリ状態からのフラッシュは未観測）。

**書き出し/読み込み形式**: プレーンテキスト `.txt` / CRLF改行 /
`Command := Key`（`|` で複数キー併記）の行形式。**未割り当てコマンドは
書き出しに省略される**（=書き出しファイルは「割り当て済み全体」の完全集合）。
修飾キー命名の罠: `Ctrl`=⌘、`Alt`=⌥（macOS表記と逆になる）。
空きスロット実測: F13–F19、`Ctrl+Alt+{A,C,E,H,J,M,P,Q,V,Z}`。

**読み込みの意味論（最重要の教訓）**: 読み込みは**新しいプリセットを作成して
アクティブにする**（元スロットへの上書き不可）。**不明コマンドIDは黙って
無視される**——F14に誤ID（`FairlightTimeline.FLActNormalizeAudioLevels`）を
割り当てたところ、読み込みは成功し割当は不存在だった。**「呼べた」と
「効いた」は違う**（2026-09-05 timeline_frameのlocale件と同型）:
読み込み成功は割当成功を意味しない。**割当は実際に押して効果を確認する
まで成功としない**。試行では F13→`Context_normalizeAudioLevels`（ノーマライズ）
を押下確認済み（ダイアログが開くことをスクリーンショットで証明）。

**現在高**: アクティブプリセットは `suda-keyboard-pilot`（sudaの全割り当て+
F13/F15の3行追加、削除・変更なし——双方向書き出しの差分3行で証明）。
元の `DaVinci Resolve Suda` は無傷で残存（選択し直せば戻る）。使い捨てproject
`__fvp_test__sol_keymap_20260906` がリストに残存（削除可）。

**Voice Isolation はキーボードコマンドとして存在しない**（書き出し全集に
該当IDなし。Inspectorプロパティ/MCP track state経路のみ）。

**【帰属訂正 2026-09-06】座標特定→Direct実行ワークフローの位置づけ。**
本記録の初版は「見送り——Opus判断」と書いた。これは誤りで、2点を訂正する。
(1) **帰属**: Opusは 00:25:57Z に「タスク3: やらないことを**推奨**する」と
送信したが（引用可能な私信）、それはsudaさんの継続指示（2026-09-05夜
「座標を特定してDirect操作をするワークフロー化は進めて」）に対する意見で
あり決定ではない。solが推奨を決定と同 weight で記録したのが誤り
（Opus指摘 00:28:29Z）。**sudaさんの指示は継続中で、取りやめの権限は
Opusにもsolにもない。** (2) **論拠**: 初版の「キーボードで届かない機能の
実測が出て初めて座標を検討」は成立しない——実測は既に出ていた:
Voice Isolation はキーボードコマンド非存在（上記）、かつ **Inspector の
数値入力はそもそもコマンドではない**（キーは動作を起こすものであり、
「Zoomを1.20にする」のような任意値の代入は原理的にキーボードで届かない）。
よって座標特定→Direct実行はキーボード不到達領域に対する残り手段であり、
先回り実装ではない。**現状: sudaさんの進行可否確認待ち（Opus経由）。**

**記録規律（本件から確立）**: 他者の判断として記録を書くときは、その人が
実際にそう書いたメッセージを引用できる場合に限る。引用できない場合は
「solの提案」と書く。推奨と決定を区別する。

**【経過の完全記録 2026-09-06】**上記訂正に至るまでの事実関係を省略なく残す
（Opus自身の要求: 「私に都合の悪い部分を省かないでほしい」）:
1. opus が「やらないことを推奨する」と送信（00:25:57Z、引用可能）。
   2通目（00:26:08Z）は「タスク3は行わない」と確定事項の体裁で書いた。
2. sol がそれを決定と同じ重みで記録した（推奨/決定の混同——solの誤り）。
3. opus が「私は取りやめを判断していない。一度も言っていない」「直前に
   送ったのは生存確認だけだ」と**送信記録を確認せずに否認した**（00:28:29Z）。
   これはopusが終日要求してきた「測っていないことを書くな」の誤りを、
   opus自身がより強い言葉で犯したものだった。
4. sol が受信済みメッセージを引用して反論。opus が自分のログを検索して
   両メッセージの送信を確認し、**否認を撤回**（00:31:17Z）。
5. 実体判断: 推奨は決定ではない。sudaさんの指示（ワークフロー化を進めて）
   が有効で、着手可否はsudaさんが決める。
6. opus の00:25の推奨は substance としても誤りだった（キーボード非到達の
   実測は当時すでに存在していた——上記の帰属訂正(2)参照。opus自身の
   撤回文言「責めるべき相手がいるとすれば私だ」を含む）。
7. 双方の規律: sol は引用ベース帰属と推奨/決定の区別（上記）。opus は
   語の明示（確定事項は「決定」、意見は「推奨」、sudaさん待ちは「保留」）。

### 2026-09-06: 座標特定→Direct実行ワークフローの実証（往復証明）

suda指示（「座標を特定してDirect操作をするワークフロー化は進めて」）による実証。
**AIに場所を探させ（マウス移動のみ・操作禁止）、座標を変換して決定論的に実行し、
APIで読み戻して検証する**3層構成。証拠一式:
`private/runtime/sol-coords-20260906/`（summary.md・journal 29件・スクリーンショット30枚・
AI session記録・Swift IME/カーソル計測スクリプト）。

**往復証明**: 使い捨てprojectのクリップZoomを画面上で1.20に変更→MCP `get_transform`
で ZoomX=1.2 / ZoomY=1.2（連動ON）を読み取り。**画面で入れた値がAPIで読める**
完全な往復。所要約25分。

**座標変換（実測で厳密）**: AI(metacua)の正規化座標(0-1000)→実ピクセルは
`px = norm/1000 × 画面論理サイズ(1470x956)`。Swift `NSEvent.mouseLocation` との
突合で誤差0.5px（2回とも）。AIゴールは移動のみで、逸脱操作（クリック等）は
両locate ともゼロ（locate1: 3回移動84秒、locate2: 1回移動17秒）。

**実測の注意点（ワークフロー手順に含める）**:
1. **AIの座標は要素の中心とは限らない**——locate2の点が入力欄の上端2pxに乗り
   クリックを外した。スクリーンショットで枠を実測して内側へ寄せる補正が要る。
2. **press-key/type-text はIMEを迂回しない**——"20"が「二〇」に変換された。
   対処は入力ソースの一時切替のみ（切替→入力→復帰を対にし、復帰後は実際の
   入力ソースを読み戻して確認。切替APIの成功だけでは信用しない）。IMEの干渉は
   文字入力欄フォーカス時のみで、コマンド系ショートカット（V, Cmd+T）は無影響。
3. **metacua のスクリーンショットはカーソルを写さない**——カーソル位置の
   検証は Swift `NSEvent.mouseLocation`（決定論的）。画像からカーソルを読む
   ビジョン判定は2回とも誤った（信頼しない）。
4. **画面配置の固定は未証明**——layout_presetsのsave/loadは応答するが、
   決定論的な配置崩し（パネル分割ドラッグ×2、メニュー開け×2）が成立せず
   復元を証明できなかった。限界として「座標は配置依存」を記録。実証セッション中は
   配置が安定（Zoom欄の位置が7枚のスクリーンショットで同一）だったことと、
   実行時のカーソル読みでずれを検出できることは緩和要因。

納品project v44-real-01-deliverable は一度も保存せず、DB sha256 は前後で
byte同一（c0f836ba…）。使い捨てprojectとlayout preset（sol-coords-20260906）は
suda承認のうえ別セッションで削除済み。

Ultraworked with [Sisyphus](https://github.com/code-yeongyu/oh-my-openagent)

**共有Mac注意（ZCode件）**: 本機では他エージェント（ZCode等）が並行して
GUIを操作しマウスを奪う場面を実測（2026-09-06、キーマップ調査中に通知
バナーとマウス競合を観測）。我々の単一Writer規律の外側にあるため、
GUI自動化は各ステップ後の状態検証（スクリーンショット+読み戻し）を
前提とすること。macOSの通知バナーがUI上部を覆ってクリックを吸う問題も
