...
|---|---|---|---|---|
| A04 | Project設定の読替・変更 | 縦型9:16（解像度・fps）の設定変更。前提条件ブロック参照 | 未検証 | 未検証（v4.3 project-timeline-creation probeはtimelineFrameRate=30のみ。解像度変更は未試行） |
| A05 | Timeline設定の読替・変更 | 縦型Timelineの tall 解像度設定。前提条件ブロック参照 | 未検証 | 未検証 |
...
| M06 | 既存ProxyのLink/Unlink | 4K縦型の重い素材を軽く扱う | 未検証 | 未検証 |
| M07 | Proxy/Optimized Media新規生成 | 同上（生成はCU route） | 未検証 | 未検証 |
...
| T02 | 新規DRT/DRPをoffline authoring | 再構築方式の切断・再配置全般 | 未検証 | 未検証（drt.assemble自体はT12速度検証で使用実績あり。カット構成用途の検証なし） |
| T03 | Track追加・削除・名前・Lock・Enable | 字幕・テロップ用V2/V3 track確保 | 未検証 | 未検証 |
| T04 | Clip/RangeのCopy・Move・Duplicate | まとめ切り抜き（wishlist #2）の切貼り | 未検証 | 未検証 |
| T05 | Ripple Insert / Timeline Ripple | 詰め編集・挿入 | 未検証 | 未検証 |
| T06 | Overwrite・Lift・Range置換 | 区間差替え | 未検証 | 未検証 |
| T07 | Blade/Razor/Split | ジャンプカット（TikTok系の高頻度技法） | 未検証 | 未検証（LiveはCU route。Offline drp.split_clipも未試行） |
| T08 | Trim/Slip/Slide/Roll | 間詰め・尺調整 | 未検証 | 未検証 |
...
| T16 | Stabilize / Smart Reframe | 手ブレ・縦型reframe | 未検証 | 未検証 |
| T17 | Compound / Fusion Clip作成 | 複合演出の1object化 | 未検証 | 未検証 |
...
| F02 | propertyのkeyframe | **スムーズなアニメatedズーム**（Transform zoom静止画はAだが動きはここ） | 未検証 | 未検証（TikTok系②の核心なのに未試行 — headline級） |
...
| F04 | Fusion node追加・削除・接続 | モーション系の下地（Blur/Transform/Merge/Mask） | 未検証 | 未検証（vendor rough-cut trap: MediaOutはMediaInからのpath必須。tool名推測禁止・probe先行） |
| F05 | Fusion parameter・animation | RPG風地点表示の速い出入り（style ③）等 | 未検証 | 未検証（入れ子カード機構は実装済みだがアニメーションは一度も作っていない — style-vocabulary.md B③） |
| F06 | Edit ResolveFX/OpenFXをclipへ追加 | glitch等の質感エフェクト付与 | 未検証 | 未検証（CU route。Fusion置換可の場合のみF04） |
| F07 | Edit FXのInspector parameter調整 | 同上の調整 | 未検証 | 未検証（CU route） |
| F08 | OFX Generator挿入 | 独立Generator clip | 未検証 | 未検証 |
| F09 | Color node内OFXをoffline編集 | Color内エフェクトの構造編集 | 未検証 | 未検証 |
| F10 | Magic Mask | 背景差替え（wishlist #4）・被写体分離（style ①モノクロ演出） | 未検証 | 未検証（subject clickはHITL必須。MCPはclick生成不能） |
...
| S02 | Titleを指定track・frameへ配置 | V2/V3への正確な重ね置き | 未検証 | 未検証（vendor trap: Insert系はtrackIndexを持たず常にV1 — issue #74） |
| S03 | 自動字幕生成 | 字幕起こしの起点 | 未検証 | 未検証（v4.3 subtitle probeはwould_generate=Trueのみで生成自体未実施。matrix #12でtimeline_ai生成4キューの副産物確認あり — 正式検証として未整理） |
| S04 | SRTからNative字幕付き新規DRT | 字幕付きTimeline再構築 | 未検証 | 未検証 |
| S05 | SRTを開いているTimelineへImport | 既存Timelineへの字幕追加 | 未検証 | 未検証（CU route。API経路なし） |
...
| S07 | Native字幕1件の開始・終了・分割・結合 | 語を切らない分割（style-vocab A節の硬制約）・word-by-word字幕の部品 | 未検証 | 未検証（CU route） |
| S08 | Subtitle track全体のstyle | 全字幕のfont/サイズ一括変更 | 未検証 | 未検証（Advanced project_db。Resolve完全終了必須） |
| S09 | Native字幕ごとのstyle/位置調整 | 1件だけの強調 styling | 未検証 | 未検証（CU route） |
| S10 | 字幕付き納品（Burn-in/embedded/sidecar） | プラットフォーム別字幕納品 | 未検証 | 未検証 |
...
| C02 | Grade Copy・Version・Restore | cut間look統一 | 未検証 | 未検証 |
| C03 | Gallery Still・LUT Export | look保存・持越し | 未検証 | 未検証 |
...
| C06 | Power Window/Qualifier/HDR/Blur/Key | 被写体だけ残し背景モノクロ（style ①）・背景ぼかし（wishlist #4系） | 未検証 | 未検証（Advanced DRX codec。calibration未確認） |
| C07 | Tracker / Color Warper | モーショントラッカー正対の地点表示（style ③） | 未検証 | 未検証（CU route・難易度高。style-vocab B③: CUでも自動化困難の可能性 — 「できる」と書かない） |
| C08 | Shot/Skin/WB/Reference match | 複数cutの色統一 | 未検証 | 未検証 |
| C09 | Scope/Gamut/Legal QC | 品質測定 | 未検証 | 未検証 |

### U. Audio・Fairlight

| 機能ID | 機能名 | TikTok系での用途 | 現在の状態 | 根拠 |
|---|---|---|---|---|
| U01 | Audio構造・mapping・level調査 | mix前の測定 | 未検証 | 未検証 |
...
| U06 | AI Audio Assistant | one-click mix | 未検証 | 未検証（どのbuildでもscript不可 — vendor issue #128） |
| U08 | BGM ducking計画・offline試聴mix | 声に合わせたBGM計画 | 未検証 | 未検証（v4.3 bgm-track-ducking probe failed: scripting APIにducking面なし） |
| U09 | BGM ducking実施・微調整・音楽sync | 声に合わせたBGM・音楽同期カット | 未検証 | 未検証（CU route。automation/sidechain書込のAPI手段なし） |
| U10 | Audio fileのsplit/trim/convert | SFXタイミング用offline加工 | 未検証 | 未検証 |
| U11 | Loudness/True Peak/LRA納品QC | 納品 loudness 測定 | 未検証 | 未検証 |
...
| R03 | Render出力の即時検証 | 生成物のspec検査 | 未検証 | 未検証（render.verify_output未使用。本日のSSIM/画素diffはad-hoc scriptであり同toolの検証ではない） |
| R04 | Deliverページ固有設定をGUI操作 | APIにないcheckbox等 | 未検証 | 未検証（CU route） |
| R05 | 納品QC・Compliance | 規格照合 | 未検証 | 未検証（v4.3 advanced-delivery-qcはsurface到達のみ。QC判定自体の検証なし） |
| R06 | 字幕出力検証 | burn-in/embedded/sidecar確認 | 未検証 | 未検証 |

### Q. 解析・編集判断（TikTok系に直結するもののみ）

| 機能ID | 機能名 | TikTok系での用途 | 現在の状態 | 根拠 |
|---|---|---|---|---|
| Q03 | Scene Cut Detection | cut検出（jump-cut補助） | 未検証 | 未検証 |
| Q06 | Editorial plan・Selects・Silence edit案 | 冒頭10秒まとめ（wishlist #2）の判断材料 | 未検証 | 未検証（v4.3 edit-engine-selects probe failed。planは編集完了ではない） |
...
| # | 変わるもの | 内容 | 状態 |
|---|---|---|---|
| V1 | Timeline解像度 | tall 解像度への設定変更（A04/A05 route。timelineFrameRateはtimeline作成後にlockされる実測 — vendor trap。解像度も同手順要確認） | 未検証 |
...
| V3 | 構図・テロップ・字幕サイズ | 全部変わる。現行承認値（テロップ2階層・字幕5.5%/W5・中央配置）は1920x1080前提（style-vocab A節＋presentation-profile-default.json） | 未検証 |
| V4 | Render設定 | 1080x1920での設定・実行（R01/R02 route）。vendorには `tiktok`/`reels`/`shorts`→`h264_vertical_1080_web`（1080x1920）のdelivery target aliasが存在する（`private/vendor/davinci-resolve-mcp/src/utils/delivery_targets.py`）が、**我々は一度も使っていない** | 未検証 |
| V5 | QC | render-qc.json 1920x1080固定の作り替え（R05/R06 route） | 未検証 |
