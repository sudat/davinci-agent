...
| S08 | Subtitle track全体のstyle | 全字幕のfont/サイズ一括変更 | 未検証 | batch C: 時間切れで未達（字幕トラック作成まで届かず）〔旧記載:  未検証（Advanced project_db。Resolve完全終了必須）〕 |
...
| N19 | クリップ端フェードハンドル | BGM/SFX出入りフェード | 未検証 | batch C実測: 端フェードハンドルのドラッグ未達（同上の競合）。メニューバーにFairlight>バッチフェード設定…/バッチフェードを適用 を確認（フェード能力の存在は確認、edit面のハンドル操作は未達）〔旧記載:  同上〕 |
