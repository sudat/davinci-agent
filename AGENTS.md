...
**開発・調査はサブエージェントに委任する。リード（Sol）自身はマネジメントと統合確認のみを行う。** コードの記述・修正・リファクタ、および本リポジトリや実行データへの本格的な調査は、原則として部隊（サブエージェント）へ委任し、リードは指示・委任結果の検証・統合コミット・受け入れ実行・報告に徹する。オペレーターから直接指示された軽微な記録作業（本項の追記など）はこの限りではない。
